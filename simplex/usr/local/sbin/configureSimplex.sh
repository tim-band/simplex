#! /bin/sh
set -eu
scfile=/usr/local/sbin/simplexctl
nxfile=/etc/nginx/conf.d/simplex.conf
max_processes=99
if [ -z "${1-}" ]
then
echo 'Usage:'
echo 'configureSimplex.sh 6'
echo 'sets simplex to use 6 processes'
echo 'configureSimplex.sh 5 3901'
echo 'sets simplex to use 5 processes on port numbers 3901 to 3906'
exit
